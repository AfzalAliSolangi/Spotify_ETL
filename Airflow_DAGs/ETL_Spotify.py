...
def spotify_etl_func():
    ...
    spotify_redirect_url = "http://127.0.0.1:5500/"
    ...
    # song_df['date_time_played'] = song_df['date_time_played'].astype(str)
    # song_df.to_csv(r"D:/Spotify_ETL-main/song_df.csv")
    #NOW We are going to Load PostgresSQL

    #Loading the data into the Temporary Table
    conn = psycopg2.connect(host = "localhost",port="5432", dbname = "SpotifyDB",password="password",user="postgres")
    cur = conn.cursor()
    engine = create_engine('postgresql+psycopg2://postgres:password@localhost:5432/SpotifyDB')
    ...
    cur_eng.execute(
    """
    INSERT INTO spotify_schema.spotify_track(unique_identifier, song_id, song_name, duration_ms, url, popularity, date_time_played, album_id, artist_id, date_time_inserted)
    SELECT t.unique_identifier,t.song_id,t.song_name,t.duration_ms,t.url,t.popularity,t.date_time_played,t.album_id,t.artist_id,%s
    FROM   public.tmp_track t
    ...
    DROP TABLE public.tmp_track;""", (datetime.datetime.now(),))
    conn_eng.commit()
    ...
    cur_eng.execute(
    """
    INSERT INTO spotify_schema.spotify_album(album_id,name,release_date,total_tracks,url)
    SELECT t.album_id,t.name,t.release_date,t.total_tracks,t.url
    FROM   public.tmp_album t
    LEFT   JOIN spotify_schema.spotify_album USING (album_id)
    WHERE  spotify_schema.spotify_album.album_id IS NULL;

    DROP TABLE public.tmp_album;""")
    ...
    cur_eng.execute(
    """
    INSERT INTO spotify_schema.spotify_artists(artist_id,name,url)
    SELECT t.artist_id,t.name,t.url
    FROM   public.tmp_artist t
    LEFT   JOIN spotify_schema.spotify_artists USING (artist_id)
    WHERE  spotify_schema.spotify_artists.artist_id IS NULL;

    DROP TABLE public.tmp_artist;""")
